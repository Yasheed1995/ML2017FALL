...
def load_data(train_data_path, train_label_path, test_data_path):
    ...
    for i in range(len(X_train)):
        a.append(np.concatenate((X_train[i], np.concatenate(([h**3 for h in X_train[i, 0:6]], [h**2 for h in X_train[i, 0:6]])))))
    b = []
    for i in range(len(X_test)):
        b.append(np.concatenate((X_test[i], np.concatenate(([h**3 for h in X_test[i, 0:6]], [h**2 for h in X_test[i, 0:6]])))))

    return (np.array(a), Y_train, np.array(b))
