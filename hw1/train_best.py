    ...
    def data_prep(self, dataset_type):
        ...
        if dataset_type == "Train":
            ...
            for i in range(12):
                for j in range(471):
                    X.append([])
                    for t in range(7,10): # just choose PM2.5 and PM10
                        for s in range(9):
            ...
            for r in row:
                ...
                elif row_count % 18 == 7 or row_count % 18 == 8 or row_count % 18 == 9:
                    for i in range(2, 11):
                        if r[i] != 'NR':
                            test_x[row_count//18].append(float(r[i]))
                        else:
...
if __name__ == '__main__':
	if sys.argv[1] == 'data/train.csv':
		theta_trained = train()
		np.save('data/model_best.npy', theta_trained)
	else: # test
		theta_trained = np.load('data/model_best.npy')
		test(theta_trained)
